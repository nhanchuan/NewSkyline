...
public class EventDAO
{
    ...
    private static string connectionString = ConfigurationManager.ConnectionStrings["connectionStrCon"].ToString();
    ...
    public static List<CalendarEvent> getEvents(DateTime start, DateTime end)
    {
        ...
        SqlCommand cmd = new SqlCommand("SELECT * FROM CalendarEvent where event_start>=@start AND event_end<=@end AND user_id=@user_id", con);
        cmd.Parameters.AddWithValue("@start", start);
        cmd.Parameters.AddWithValue("@end", end);
        cmd.Parameters.AddWithValue("@user_id", HttpContext.Current.Session.GetCurrentUser().UserID);
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("UPDATE CalendarEvent SET title=@title, description=@description WHERE event_id=@event_id AND user_id=@user_id", con);
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("UPDATE CalendarEvent SET event_start=@event_start, event_end=@event_end WHERE event_id=@event_id AND user_id=@user_id", con);
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("DELETE FROM CalendarEvent WHERE (event_id = @event_id) AND (user_id=@user_id)", con);
        cmd.Parameters.AddWithValue("@event_id", id);
        cmd.Parameters.AddWithValue("@user_id", HttpContext.Current.Session.GetCurrentUser().UserID);
        using (con)
        {
    ...
    public static int addEvent(CalendarEvent cevent)
    {
        ...
        //insert
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("INSERT INTO CalendarEvent(title, description, event_start, event_end, user_id) VALUES(@title, @description, @event_start, @event_end, @user_id)", con);
        ...
        using (con)
        {
            ...
            cmd = new SqlCommand("SELECT max(event_id) FROM CalendarEvent where title=@title AND description=@description AND event_start=@event_start AND event_end=@event_end AND user_id=@user_id", con);
            cmd.Parameters.AddWithValue("@title", cevent.title);
            cmd.Parameters.AddWithValue("@description", cevent.description);
            cmd.Parameters.AddWithValue("@event_start", cevent.start);
            cmd.Parameters.AddWithValue("@event_end", cevent.end);
