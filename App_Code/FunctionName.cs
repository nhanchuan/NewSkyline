using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

/// <summary>
/// Summary description for FunctionName
/// </summary>
public static class FunctionName
{
    public static string NewUser = "FC001";
    public static string AllUser= "FC002";
    public static string UserGroups= "FC003";
    public static string MenuSetting= "FC004";
    public static string FileExplorer= "FC005";
    public static string ImagesManager= "FC006";
    public static string NewImages= "FC007";
    public static string CategoryImages= "FC008";
    public static string VideoManager= "FC009";
    public static string NewVideo= "FC010";
    public static string CategoryVideo= "FC011";
    public static string PostManager= "FC012";
    public static string NewPost= "FC013";
    public static string CategoryManager= "FC014";
    public static string TagManager= "FC015";
    public static string Advisory= "FC016";
    public static string AdvisoryManager= "FC017";
    public static string TuVanVien= "FC018";
    public static string HoSoTongHop= "FC019";
    public static string ThuLyHoSo= "FC020";
    public static string CacTruongLienKet= "FC021";
    public static string QLGhiDanh= "FC022";
    //public static string QLHocPhi= "FC023";
    public static string DiemDanh= "FC024";
    //public static string QLKhoiLop= "FC025";
    //public static string QLCapDo= "FC026";
    //public static string QLLopHoc= "FC027";
    public static string QLHocVien= "FC028";
    public static string QLGioHoc= "FC029";
    public static string QLGiaoTrinh= "FC030";
    public static string QLGiaoVien= "FC031";
    public static string QLChiNhanh= "FC032";
    public static string QLCoso= "FC033";
    public static string QLPhongHoc= "FC034";
    public static string QLSinhNhat= "FC035";
    public static string PostEdit= "FC036";
    public static string VideoEdit= "FC037";
    public static string ThongTinHocVien= "FC038";
    public static string GhiDanhHocVien= "FC039";
    public static string History= "FC040";
    //public static string DiemDanh= "FC041";
    public static string PhieuGhiDanh= "FC042";
    public static string ThongTinHoSoKhachHang= "FC043";
    public static string UpdateThongTinHoSoKhachHang = "FC044";
    public static string KiemTraPhieuChuyen= "FC045";
    public static string HoSoKhachHang= "FC046";
    public static string PhanCongHoSo= "FC047";
    public static string PhieuDKTuVan= "FC048";
    public static string ThongtinPhieuDKTuVan = "FC049";
    public static string QLDangKyTuVan= "FC050";
    public static string ThongTinKhachHang= "FC051";
    public static string ThongTinThanhVien = "FC052";

    public static string TraCuuHoSo = "FC053";
    public static string QuanLyHocVien = "FC054";
    public static string LoaiHinhDaoTao = "FC055";
    public static string LoaiChuongTrinhDaoTao = "FC056";
    public static string ChuongTrinhDaoTao = "FC057";
    public static string LopHoc = "FC058";
    public static string KhoaHoc = "FC059";
    public static string GhiDanhKhoaHoc = "FC060";
    public static string NhapHoSo = "FC061";

}